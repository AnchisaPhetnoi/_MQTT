...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        msg_id = esp_mqtt_client_subscribe(client, "KMITL/SIET/65030289/LED", 0);
...
static void button_task(void *arg)
{
    while (gpio_get_level(BUTTON_GPIO) == 1) {  // While button is pressed
        if (!button_pressed) {
            button_pressed = true;
            int msg_id = esp_mqtt_client_publish(client, "KMITL/SIET/65030289/Button", "1", 0, 1, 0);
    ...
    if (button_pressed) {
        ...
        int msg_id = esp_mqtt_client_publish(client, "KMITL/SIET/65030289/Button", "0", 0, 1, 0);
